...
import { initSales, initInventory, initPayments, initReceivables, initFabric, FORM_SCHEMAS } from '../data/seedData'
...
export default function AdminDashboard({ activeModule = 'dashboard' }) {
  ...
  const addSampleData = async (tabKey) => {
    const sampleMap = {
      sales:       initSales.slice(0, 5),
      inventory:   initInventory.slice(0, 5),
      payments:    initPayments.slice(0, 5),
      receivables: initReceivables.slice(0, 5),
      fabric:      initFabric.slice(0, 5),
    }
    const tbl = tables[tabKey]
    const samples = sampleMap[tabKey]
    if (!tbl || !samples) return
    ...
    try {
      ...
      alert(`Error adding samples: ${err.message}`)
    } finally {
  ...
  const pendingPay    = filteredPay.filter(r => r.status === 'Pending' || r.status === 'Overdue').reduce((s, r) => s + Number(r.pending_amount || 0), 0)
  ...
  const todaySalesRows  = sales.rows.filter(r => { const raw = r.date || r.created_at || ''; return raw && new Date(raw).toDateString() === _today.toDateString() })
  const thisMonthSales  = sales.rows.filter(r => { const raw = r.date || r.created_at || ''; if (!raw) return false; const d = new Date(raw); return d.getFullYear() === _today.getFullYear() && d.getMonth() === _today.getMonth() })
  const todayOrders     = todaySalesRows.length
  const todayRevenue    = todaySalesRows.reduce((s, r) => s + Number(r.amount || 0), 0)
  const monthlyRevenue  = thisMonthSales.reduce((s, r) => s + Number(r.amount || 0), 0)
  const pendingOrders   = filteredSales.filter(r => r.status === 'Pending' || r.status === 'Open').length
  const completedOrders = filteredSales.filter(r => r.status === 'Completed').length
  const overdueOrders   = filteredSales.filter(r => r.status === 'Overdue').length
  // Payment breakdown
  const paidRevenue     = filteredPay.filter(r => r.status === 'Paid').reduce((s, r) => s + Number(r.paid_amount || r.total_amount || 0), 0)
  const payPending      = filteredPay.filter(r => r.status === 'Pending').length
  const payOverdue      = filteredPay.filter(r => r.status === 'Overdue').length
  const payPaid         = filteredPay.filter(r => r.status === 'Paid').length
  ...
  if (activeModule !== 'dashboard') {
    ...
    const subFiltered = {
      ...
      receivables: filteredRecv,
      fabric: filteredFab,
    }
    const subRows = subFiltered[activeModule] || tbl.rows
    const hasDateFilter = activeModule !== 'inventory'
    const SUB_RANGES = [{ key:'all', label:'All' }, { key:'today', label:'Today' }, { key:'7d', label:'7 Days' }, { key:'30d', label:'30 Days' }]
    ...
    return (
      <div className="fade-up">
        ...
        <Panel>
          <DataTable
            columns={cols} rows={subRows} loading={tbl.loading}
  ...
  return (
    <div className="fade-up">
      ...
      {/* Number of Orders */}
      <div style={{ fontSize:10, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.1em', fontWeight:700, marginBottom:7 }}>Number of Orders</div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,minmax(0,1fr))', gap:10, marginBottom:14 }}>
        <StatCard icon={ShoppingBag}   label="Total Orders"      value={<AnimatedCounter value={totOrders}     prefix="" />} change="in selected period"   up={true}  accentColor="var(--admin)" />
        <StatCard icon={TrendingUp}    label="Today's Orders"    value={<AnimatedCounter value={todayOrders}   prefix="" />} change="placed today"         up={true}  accentColor="var(--green)" />
        <StatCard icon={Clock}         label="Pending Orders"    value={<AnimatedCounter value={pendingOrders} prefix="" />} change="awaiting fulfillment" up={false} accentColor="var(--amber)" />
        <StatCard icon={ArrowUpRight}  label="Completed Orders"  value={<AnimatedCounter value={completedOrders} prefix="" />} change="fulfilled"         up={true}  accentColor="var(--green)" />
        <StatCard icon={ArrowDownRight} label="Overdue / Cancelled" value={<AnimatedCounter value={overdueOrders} prefix="" />} change="past due date"    up={false} accentColor="var(--red)"   />
      </div>

      {/* Payment Received */}
      <div style={{ fontSize:10, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.1em', fontWeight:700, marginBottom:7 }}>Payment Received</div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,minmax(0,1fr))', gap:10, marginBottom:14 }}>
        <StatCard icon={DollarSign}  label="Total Revenue"    value={<AnimatedCounter value={totRevenue} />}     change="in selected period"    up={true}  accentColor="var(--admin)" />
        <StatCard icon={DollarSign}  label="Today's Revenue"  value={<AnimatedCounter value={todayRevenue} />}   change="earned today"          up={true}  accentColor="var(--green)" />
        <StatCard icon={TrendingUp}  label="Monthly Revenue"  value={<AnimatedCounter value={monthlyRevenue} />} change="this calendar month"   up={true}  accentColor="var(--ceo)"   />
        <StatCard icon={Clock}       label="Pending Payments" value={<AnimatedCounter value={pendingPay} />}     change={`${payPending} invoices pending`}  up={false} accentColor="var(--amber)" />
        <StatCard icon={ArrowUpRight} label="Payment Status"  value={<AnimatedCounter value={paidRevenue} />}    change={`${payPaid} paid · ${payOverdue} overdue`} up={true} accentColor="var(--green)" />
      </div>
      ...
      {/* All 5 data tables in one tabbed section */}
      {(() => {
        const DATA_TABS = [
          { key:'sales',       label:'Sales',         rows:filteredSales,  tbl:sales,  cols:COLS.sales,       schema:'sales_entries',     title:'Sales Entry'    },
          { key:'inventory',   label:'Inventory',     rows:inv.rows,       tbl:inv,    cols:COLS.inventory,   schema:'inventory',         title:'Inventory'      },
          { key:'payments',    label:'Payments',      rows:filteredPay,    tbl:pay,    cols:COLS.payments,    schema:'supplier_payments', title:'Payments'       },
          { key:'receivables', label:'Receivables',   rows:filteredRecv,   tbl:recv,   cols:COLS.receivables, schema:'receivables',       title:'Receivables'    },
          { key:'fabric',      label:'Fabric Orders', rows:filteredFab,    tbl:fabric, cols:COLS.fabric,      schema:'fabric_orders',     title:'Fabric Orders'  },
        ]
        const active = DATA_TABS.find(t => t.key === dataTab) || DATA_TABS[0]
        return (
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, overflow:'hidden', marginBottom:12 }}>
            {/* Tab bar */}
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 16px', borderBottom:'1px solid var(--border)', background:'var(--surface2)' }}>
              <div style={{ display:'flex', gap:0 }}>
                {DATA_TABS.map(t => (
                  <button key={t.key} onClick={() => setDataTab(t.key)}
                    style={{
                      padding:'10px 16px', fontSize:12, fontWeight: dataTab===t.key ? 600 : 400,
                      color: dataTab===t.key ? 'var(--text)' : 'var(--text2)',
                      background:'transparent', border:'none', cursor:'pointer', fontFamily:'inherit',
                      borderBottom: dataTab===t.key ? '2px solid var(--admin)' : '2px solid transparent',
                      transition:'all 0.15s',
                    }}
                    onMouseEnter={e => { if(dataTab!==t.key) e.currentTarget.style.color='var(--text)' }}
                    onMouseLeave={e => { if(dataTab!==t.key) e.currentTarget.style.color='var(--text2)' }}
                  >
                    {t.label}
                    <span style={{ marginLeft:6, fontSize:10, fontWeight:500, color:'var(--text3)', background:'var(--surface3)', borderRadius:10, padding:'1px 6px' }}>
                      {t.rows.length}
                    </span>
                  </button>
                ))}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <button onClick={() => addSampleData(dataTab)} disabled={addingSamples}
                  style={{
                    display:'flex', alignItems:'center', gap:5,
                    height:26, padding:'0 10px', borderRadius:5, cursor: addingSamples ? 'not-allowed' : 'pointer',
                    fontSize:11, fontWeight:500, fontFamily:'inherit',
                    border:'1px solid var(--border)', background:'var(--surface3)',
                    color:'var(--text2)', opacity: addingSamples ? 0.6 : 1, transition:'all 0.15s',
                  }}
                  onMouseEnter={e => { if (!addingSamples) { e.currentTarget.style.borderColor='var(--border2)'; e.currentTarget.style.color='var(--text)' } }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.color='var(--text2)' }}
                >
                  {addingSamples ? 'Adding…' : '+ 5 Samples'}
                </button>
<button onClick={() => openAdd(active.key)}
                  style={{ fontSize:11.5, color:'var(--admin)', background:'transparent', border:'none', cursor:'pointer', fontWeight:600, fontFamily:'inherit', padding:'4px 0' }}
                >+ Add {active.title}</button>
              </div>
            </div>
            {/* Table */}
            <DataTable
              columns={active.cols} rows={active.rows} loading={active.tbl.loading}
              highlightQuery={highlight}
              onAdd={() => openAdd(active.key)}
              onEdit={row => openEdit(active.key, row)}
              onDelete={id => active.tbl.remove(id)}
              onExportPDF={() => exportToPDF(active.title, active.cols, active.tbl.rows)}
              onExportExcel={() => exportToExcel(active.title, active.cols, active.tbl.rows)}
            />
          </div>
        )
      })()}
