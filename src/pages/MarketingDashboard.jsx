...
import { Target, TrendingUp, Users, BarChart2, DollarSign, Calendar, ChevronLeft, ChevronRight, Mail, MessageSquare, MousePointerClick, Eye, ShoppingCart, AlertTriangle } from 'lucide-react'
...
import { initGoogleAds, initMetaAds, initCommunicationAds, initAbandonedCarts, FORM_SCHEMAS } from '../data/seedData'
...
const CART_COLS = [{ key:'customer',label:'Customer' },{ key:'product',label:'Product' },{ key:'cart_value',label:'Cart Value' },{ key:'recovery_channel',label:'Channel' },{ key:'status',label:'Status' }]
...
export default function MarketingDashboard({ activeModule = 'dashboard' }) {
  ...
  const carts  = useTable('abandoned_carts', initAbandonedCarts)

  useEffect(() => {
    const handleSearch = (e) => {
      const { module, query, id, record } = e.detail
      const marketingModules = ['google', 'meta', 'comm', 'roi', 'abandoned', 'dashboard']
  ...
  const filteredCarts  = useMemo(() => filterRows(carts.rows,  range, selectedDate), [carts.rows,  range, selectedDate])

  // Abandoned cart metrics
  const recoveredCarts  = useMemo(() => filteredCarts.filter(r => r.status === 'Recovered'), [filteredCarts])
  const abandonedCarts  = useMemo(() => filteredCarts.filter(r => r.status === 'Abandoned'), [filteredCarts])
  const pendingCarts    = useMemo(() => filteredCarts.filter(r => r.status === 'Pending'),   [filteredCarts])
  const totalCarts      = filteredCarts.length
  const recoveryRate    = totalCarts > 0 ? ((recoveredCarts.length / totalCarts) * 100).toFixed(1) : '0.0'
  const recoveredRevenue = useMemo(() => recoveredCarts.reduce((s, r) => s + Number(r.cart_value || 0), 0), [recoveredCarts])
  const pendingRecovery  = useMemo(() => pendingCarts.reduce((s, r) => s + Number(r.cart_value || 0), 0),   [pendingCarts])

  // Abandoned cart trend — group all-time data by month
  const cartTrendData = useMemo(() => {
    const map = {}
    carts.rows.forEach(r => {
      const raw = r.created_at || ''; if (!raw) return
      const d = new Date(raw)
      const key = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`
      const lbl = d.toLocaleDateString('en-IN', { month:'short' })
      if (!map[key]) map[key] = { month:lbl, ts:new Date(d.getFullYear(),d.getMonth(),1).getTime(), abandoned:0, recovered:0, pending:0 }
      if (r.status === 'Recovered')    map[key].recovered++
      else if (r.status === 'Abandoned') map[key].abandoned++
      else                              map[key].pending++
    })
    return Object.values(map)
      .sort((a,b) => a.ts - b.ts)
      .map(({ month, abandoned, recovered, pending }) => ({ month, abandoned, recovered, pending }))
  }, [carts.rows])

  // Recovery by channel
  const cartChannelData = useMemo(() => {
    const map = {}
    filteredCarts.forEach(r => {
      const ch = r.recovery_channel || 'Other'
      if (!map[ch]) map[ch] = { channel:ch, total:0, recovered:0 }
      map[ch].total++
      if (r.status === 'Recovered') map[ch].recovered++
    })
    return Object.values(map)
  }, [filteredCarts])
    ...
    return (
      <div className="fade-up">
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
          <div><h1 style={{ fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:'var(--text)' }}>Google Ads</h1><p style={{ fontSize:12,color:'var(--text2)',marginTop:3 }}>Search · Display · Shopping campaigns</p></div>
          <div style={{ display:'flex', gap:3, flexShrink:0 }}>
            {RANGES.map(r => <button key={r.key} onClick={() => { setRange(r.key); if(r.key==='today') setSelectedDate(new Date()) }} style={{ height:28, padding:'0 11px', borderRadius:6, cursor:'pointer', fontSize:11.5, fontWeight:500, fontFamily:'inherit', border:`1px solid ${range===r.key?'var(--border2)':'var(--border)'}`, background:range===r.key?'var(--surface2)':'transparent', color:range===r.key?'var(--text)':'var(--text2)', transition:'all 0.15s ease' }}>{r.label}</button>)}
          </div>
        </div>
  ...
  if (activeModule === 'meta') {
    ...
    return (
      <div className="fade-up">
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
          <div><h1 style={{ fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:'var(--text)' }}>Meta Ads</h1><p style={{ fontSize:12,color:'var(--text2)',marginTop:3 }}>Facebook · Instagram campaigns</p></div>
          <div style={{ display:'flex', gap:3, flexShrink:0 }}>
            {RANGES.map(r => <button key={r.key} onClick={() => { setRange(r.key); if(r.key==='today') setSelectedDate(new Date()) }} style={{ height:28, padding:'0 11px', borderRadius:6, cursor:'pointer', fontSize:11.5, fontWeight:500, fontFamily:'inherit', border:`1px solid ${range===r.key?'var(--border2)':'var(--border)'}`, background:range===r.key?'var(--surface2)':'transparent', color:range===r.key?'var(--text)':'var(--text2)', transition:'all 0.15s ease' }}>{r.label}</button>)}
          </div>
        </div>
  ...
  if (activeModule === 'comm') {
    ...
    const cRevenue    = filteredComm.reduce((s,r)=>s+Number(r.revenue||0),0)
    return (
      <div className="fade-up">
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
          <div><h1 style={{ fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:'var(--text)' }}>Communication Ads</h1><p style={{ fontSize:12,color:'var(--text2)',marginTop:3 }}>WhatsApp · Email · SMS campaigns</p></div>
          <div style={{ display:'flex', gap:3, flexShrink:0 }}>
            {RANGES.map(r => <button key={r.key} onClick={() => { setRange(r.key); if(r.key==='today') setSelectedDate(new Date()) }} style={{ height:28, padding:'0 11px', borderRadius:6, cursor:'pointer', fontSize:11.5, fontWeight:500, fontFamily:'inherit', border:`1px solid ${range===r.key?'var(--border2)':'var(--border)'}`, background:range===r.key?'var(--surface2)':'transparent', color:range===r.key?'var(--text)':'var(--text2)', transition:'all 0.15s ease' }}>{r.label}</button>)}
          </div>
        </div>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:10,marginBottom:14 }}>
          ...
          <StatCard icon={DollarSign} label="Revenue"       value={fmtCur(cRevenue)}       change="from comms campaigns" up={true} accentColor="var(--mkt)" />
        </div>
        <Panel>
        ...
        {modal && <DataModal title={editing?'Edit Campaign':'Add Campaign'} schema={FORM_SCHEMAS.communication_ads} initial={editing} onSave={handleSave} onClose={closeModal} />}
      </div>
    )
  }

  // ── Abandoned Cart sub-page ──
  if (activeModule === 'abandoned') {
    return (
      <div className="fade-up">
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
          <div>
            <h1 style={{ fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:'var(--text)' }}>Abandoned Cart</h1>
            <p style={{ fontSize:12,color:'var(--text2)',marginTop:3 }}>Recovery tracking — WhatsApp · Email · SMS</p>
          </div>
          <div style={{ display:'flex', gap:3, flexShrink:0 }}>
            {RANGES.map(r => <button key={r.key} onClick={() => { setRange(r.key); if(r.key==='today') setSelectedDate(new Date()) }} style={{ height:28, padding:'0 11px', borderRadius:6, cursor:'pointer', fontSize:11.5, fontWeight:500, fontFamily:'inherit', border:`1px solid ${range===r.key?'var(--border2)':'var(--border)'}`, background:range===r.key?'var(--surface2)':'transparent', color:range===r.key?'var(--text)':'var(--text2)', transition:'all 0.15s ease' }}>{r.label}</button>)}
          </div>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:10,marginBottom:14 }}>
          <StatCard icon={ShoppingCart}  label="Total Abandoned"   value={fmtNum(totalCarts)}          change="in selected period"                                   up={false} accentColor="var(--amber)"  />
          <StatCard icon={TrendingUp}    label="Recovery Rate"     value={`${recoveryRate}%`}           change={`${recoveredCarts.length} of ${totalCarts} recovered`} up={true}  accentColor="var(--green)"  />
          <StatCard icon={DollarSign}    label="Recovered Revenue" value={fmtCur(recoveredRevenue)}     change="from recovered carts"                                  up={true}  accentColor="var(--mkt)"    />
          <StatCard icon={AlertTriangle} label="Pending Recovery"  value={fmtCur(pendingRecovery)}      change="value at risk"                                         up={false} accentColor="var(--red)"    />
        </div>

        <div style={{ display:'grid',gridTemplateColumns:'1.5fr 1fr',gap:12,marginBottom:12 }}>
          <Panel title="Abandoned Cart Trend" subtitle="Monthly breakdown — Recovered vs Abandoned vs Pending">
            <div style={{ padding:'12px 16px 14px' }}>
              {cartTrendData.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={cartTrendData}>
                    ...
                    <Bar dataKey="recovered" name="Recovered" fill="var(--green)" radius={[4,4,0,0]} maxBarSize={24} />
                    <Bar dataKey="abandoned" name="Abandoned" fill="var(--red)"   radius={[4,4,0,0]} maxBarSize={24} />
                    <Bar dataKey="pending"   name="Pending"   fill="var(--amber)" radius={[4,4,0,0]} maxBarSize={24} />
          ...
          <Panel title="Recovery by Channel" subtitle={totalCarts > 0 ? `${totalCarts} carts — channel performance` : 'No data for this period'}>
            <div style={{ padding:'14px 16px', display:'flex', flexDirection:'column', gap:10 }}>
              {cartChannelData.length > 0 ? cartChannelData.map(ch => {
                const rate = ch.total > 0 ? ((ch.recovered / ch.total) * 100).toFixed(0) : 0
                return (
                  <div key={ch.channel}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                      <span style={{ fontSize:12, color:'var(--text2)' }}>{ch.channel}</span>
                      <span style={{ fontSize:12, fontWeight:600, color:'var(--text)' }}>{ch.recovered}/{ch.total} <span style={{ color:'var(--text3)', fontWeight:400 }}>({rate}%)</span></span>
                    </div>
                    <div style={{ height:4, background:'var(--surface3)', borderRadius:3, overflow:'hidden' }}>
                      <div style={{ height:'100%', width:`${rate}%`, background:'var(--green)', borderRadius:3, transition:'width 0.8s ease' }} />
                    </div>
                  </div>
                )
              }) : (
                <div style={{ color:'var(--text3)',fontSize:13 }}>No channel data for this period</div>
              )}
            </div>
          </Panel>
        </div>

        <Panel title="Abandoned Carts" subtitle={`${filteredCarts.length} record${filteredCarts.length!==1?'s':''} — ${recoveredCarts.length} recovered · ${abandonedCarts.length} abandoned · ${pendingCarts.length} pending`}>
          <DataTable
            columns={CART_COLS} rows={filteredCarts} loading={carts.loading}
            highlightQuery={highlight}
            onAdd={() => { setEditing(null); setModal('carts') }}
            onEdit={row => { setEditing(row); setModal('carts') }}
            onDelete={id => carts.remove(id)}
            onExportPDF={() => exportToPDF('Abandoned Carts', CART_COLS, filteredCarts)}
            onExportExcel={() => exportToExcel('Abandoned Carts', CART_COLS, filteredCarts)}
          />
        </Panel>
        {modal === 'carts' && (
          <DataModal
            title={editing ? 'Edit Abandoned Cart' : 'Add Abandoned Cart'}
            schema={FORM_SCHEMAS.abandoned_carts}
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{ fontSize:10, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.1em', fontWeight:700, marginBottom:7 }}>Abandoned Cart</div>
      ...
      {modal && (
        <DataModal
          title={modal === 'carts' ? (editing ? 'Edit Abandoned Cart' : 'Add Abandoned Cart') : (editing ? 'Edit Campaign' : 'Add Campaign')}
          schema={FORM_SCHEMAS[modal === 'google' ? 'google_ads' : modal === 'meta' ? 'meta_ads' : modal === 'carts' ? 'abandoned_carts' : 'communication_ads']}
          initial={editing} onSave={handleSave} onClose={closeModal}
        />
      )}
    </div>
